...
<form class="search-form   job_search_form  js-search-form" action="<?php echo listable_get_listings_page_url(); ?>" method="get" role="search">
	...
	<div class="search_jobs  search_jobs--frontpage<?php if ( 1 == $fields_num ) echo '  has--one-field'; ?>">
		...
		<div class="search-field-wrapper  search-filter-wrapper<?php echo $has_search_menu ? '  has--menu' : ''; ?>">
			...
			<input class="search-field  js-search-suggestions-field" type="text" name="search_keywords" id="search_keywords" placeholder="<?php esc_html_e( 'Procure por um projeto', 'listable' ); ?>" autocomplete="off" value="<?php the_search_query(); ?>"/>
		...
		<?php if ( in_array( 'location', $fields_options ) ): ?>
			...
			<div class="search_location  search-filter-wrapper">
				...
				<?php } else { ?>
					<input type="text" name="search_location" id="search_location" placeholder="<?php esc_attr_e( 'Bairro', 'listable' ); ?>" />
				<?php } ?>
			</div>
        ...
        <div class="search_categories  search-filter-wrapper">
            ...
            <?php job_manager_dropdown_categories( array( 'taxonomy' => 'job_listing_category', 'hierarchical' => 1, 'show_option_all' => esc_html__( 'Categoria', 'listable' ), 'name' => 'search_categories', 'orderby' => 'name', 'multiple' => false ) ); ?>
        </div>
		...
		<button class="search-submit" name="submit" id="searchsubmit">
			<?php get_template_part( 'assets/svg/search-icon-svg'); ?>
			<span><?php esc_html_e( 'Procurar', 'listable' ); ?></span>
		</button>
	</div>
